...
def qa_llm():
    ...
    embeddings = SentenceTransformerEmbeddings(model_name="all-MiniLM-L6-v2")
    client = chromadb.PersistentClient(settings=CHROMA_SETTINGS, path="db")
    db = Chroma(client=client, persist_directory="db", embedding_function=embeddings, client_settings=CHROMA_SETTINGS)
...
def main():
    ...
    st.markdown("<h4 style='text-align: center; color: grey;'>Built by Saikiran </h4>", unsafe_allow_html=True)
